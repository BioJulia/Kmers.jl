...
[k-mer](https://en.wikipedia.org/wiki/K-mer), a biological sequence of exact length `k`.
...
In Kmers.jl, the `Kmer` type is parameterized by its length, and its data is stored in an `NTuple`. This makes `Kmer`s bitstypes and highly efficient.
...
## Usage
### ⚠️ WARNING ⚠️
`Kmer`s are parameterized by their length. That means any operation on `Kmer`s that changes their length, such as `push`, `pop`, slicing, or masking (logical indexing), will be **type unstable** and hence slow and memory inefficient, unless you write your code in such a way that the compiler can use constant folding.

Further, as `Kmer`s are immutable and their operations are aggressively inlined and unrolled,
they become inefficient as they get longer.
...
## Installation
You can install Kmers from the Julia
...
Take a look at the [contributing files](https://github.com/BioJulia/Contributing) for
detailed contributor and maintainer guidelines and the code of conduct.
