...
## FAQ
### Why can't kmers be compared to biosequences?
...
This is a consequence of the following limitations:
...
Any iterator of nucleotide kmers can be made into a canonical kmer iterator by simply calling `canonical` on its output kmers.

The `CanonicalKmers` iterator is special-cased because, with a step size of 1, it is generally faster to build the next kmer by storing both the reverse and forward kmer, then creating the next kmer by prepending/appending the next symbol.
...
The concept of kmers has turned out to be remarkably flexible and useful in bioinformatics, and has spawned a never-ending stream of variations.
We simply can't implement them all.

However, see the section [Building kmer replacements](@ref replacements) for how to implement them
