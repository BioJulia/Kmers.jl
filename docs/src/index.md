...
!!! warning
    ...
    That means any operation on `Kmer`s that changes their length, such as `push`,
    `pop`, slicing, or masking (logical indexing) will be **type unstable**
    and hence slow and memory inefficient,
    unless you write your code in such a way that the compiler can use constant folding.
