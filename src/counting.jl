...
count_a(x::Kmer{A, K, N}) where {A <: NucleicAcidAlphabet, K, N} =
    _count_a(A(), x.data...) - n_unused(x)
...
count_gc(x::Kmer{A, K, N}) where {A <: NucleicAcidAlphabet, K, N} =
    _count_gc(A(), x.data...)
Base.count(::typeof(isGC), x::Kmer{A, K, N}) where {A <: NucleicAcidAlphabet, K, N} =
    count_gc(x)

# TODO: Expand to Amino Acid Kmers as well...
@inline function Base.count(
    ::typeof(!=),
    a::Kmer{A, K, N},
    b::Kmer{A, K, N},
) where {A <: NucleicAcidAlphabet, K, N}
    ad = a.data
    bd = b.data
    sum = 0
