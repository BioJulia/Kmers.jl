...
@inline Base.IteratorSize(
    ::Type{<:AbstractKmerIterator{Kmer{A, K, N}, S}},
) where {A, S <: BioSequence{A}, K, N} = Base.HasLength()
@inline Base.IteratorSize(
    ::Type{<:AbstractKmerIterator{Kmer{A, K, N}, S}},
) where {A, B, S <: BioSequence{B}, K, N} = Base.SizeUnknown()

@inline function Base.length(
    it::AbstractKmerIterator{Kmer{A, K, N}, S},
) where {A, K, N, S <: BioSequence{A}}
...
@inline function Base.iterate(
    it::AbstractKmerIterator{Kmer{A, K, N}, LongSequence{A}},
) where {A, K, N}
    ...
    if isnothing(fwkmer)
        ...
        rvkmer = rightshift_carry(
            _reverse(
                BioSequences.BitsPerSymbol(alph),
                _complement_bitpar(alph, fwkmer...)...,
            ),
            rshift,
        )
        return KmerAt{Kmer{A, K, N}}(1, Kmer{A, K, N}(fwkmer), Kmer{A, K, N}(rvkmer)),
        (K, fwkmer, rvkmer)
    end
end
