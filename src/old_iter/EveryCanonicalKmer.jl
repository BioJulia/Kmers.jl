...
"""
struct EveryCanonicalKmer{T <: Kmer, S <: BioSequence{<:NucleicAcidAlphabet}} <:
       AbstractKmerIterator{T, S}
    seq::S
    start::Int
    stop::Int

    function EveryCanonicalKmer{T, S}(
        seq::S,
...
@inline function Base.iterate(
    it::EveryCanonicalKmer{Kmer{A, K, N}, LongSequence{A}},
) where {A, K, N}
    ...
    if isnothing(fwkmer)
        ...
        return (it.start, Kmer{A, K, N}(min(fwkmer, rvkmer))),
        (it.start + K - 1, fwkmer, rvkmer)
    end
end
...
@inline Base.IteratorSize(
    ::Type{<:EveryCanonicalKmer{Kmer{A, N, K}, LongSequence{B}}},
) where {A <: NucleicAcidAlphabet{2}, N, K, B <: NucleicAcidAlphabet{4}} =
    Base.SizeUnknown()

@inline function Base.iterate(
    it::EveryCanonicalKmer{Kmer{A, K, N}, LongSequence{B}},
    ...
    while i ≤ it.stop
        ...
        if filled == K
            return (i - K + 1, min(Kmer{A, K, N}(fwkmer), Kmer{A, K, N}(rvkmer))),
            (i, filled, fwkmer, rvkmer)
        end
        i += 1
    end
    return nothing
