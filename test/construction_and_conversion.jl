...
@testset "Construction and Conversions" begin
    ...
    @test Kmers.kmertype(Kmer{DNAAlphabet{2}, 31})((i for i in rand(ACGT, 31))) isa
          Kmers.kmertype(Kmer{DNAAlphabet{2}, 31})
    ...
    function check_longsequence_construction(
        ::Type{T},
        seq::S,
    ) where {T <: Kmer, S <: LongSequence}
        return S(T(seq)) == seq
    end
    ...
    function check_roundabout_construction(
        ::Type{T},
        A2,
        seq::AbstractString,
    ) where {T <: Kmer}
    ...
    @testset "Kmer conversion" begin
        for len in [1, 16, 32, 64, 128]
            ...
            @test all(
                Bool[
                    check_string_construction(DNAKmer{len}, random_dna_kmer(len)) for
                    _ in 1:reps
                ],
            )
            @test all(
            ...
            @test all(
                Bool[
                    check_string_construction(RNAKmer{len}, random_rna_kmer(len)) for
                    _ in 1:reps
                ],
            )
            @test all(
            ...
            @test all(
                Bool[
                    check_longsequence_construction(
                        Kmer{DNAAlphabet{2}, len},
                        LongDNA{2}(random_dna_kmer(len)),
                    ) for _ in 1:reps
                ],
            )
            @test all(
            ...
            @test all(
                Bool[
                    check_longsequence_construction(
                        Kmer{RNAAlphabet{2}, len},
                        LongRNA{2}(random_rna_kmer(len)),
                    ) for _ in 1:reps
                ],
            )
            @test all(
            ...
            @test all(
                Bool[
                    check_longsequence_construction(AAKmer{len}, LongAA(random_aa(len))) for
                    _ in 1:reps
                ],
            )
            ...
            @test all(
                Bool[
                    check_biosequence_construction(
                        Kmer{DNAAlphabet{2}, len},
                        LongSequence{DNAAlphabet{2}}(random_dna_kmer(len)),
                    ) for _ in 1:reps
                ],
            )
            @test all(
                Bool[
                    check_biosequence_construction(
                        Kmer{DNAAlphabet{4}, len},
                        LongSequence{DNAAlphabet{4}}(random_dna_kmer(len)),
                    ) for _ in 1:reps
                ],
            )
            @test all(
            ...
            @test all(
                Bool[
                    check_biosequence_construction(
                        Kmer{RNAAlphabet{2}, len},
                        LongSequence{RNAAlphabet{2}}(random_rna_kmer(len)),
                    ) for _ in 1:reps
                ],
            )
            @test all(
                Bool[
                    check_biosequence_construction(
                        Kmer{RNAAlphabet{4}, len},
                        LongSequence{RNAAlphabet{4}}(random_rna_kmer(len)),
                    ) for _ in 1:reps
                ],
            )
            @test all(
            ...
            @test all(
                Bool[
                    check_biosequence_construction(
                        AAKmer{len},
                        LongSequence{AminoAcidAlphabet}(random_aa(len)),
                    ) for _ in 1:reps
                ],
            )
            ...
            @test all(
                Bool[
                    check_longsequence_construction(Kmer, LongRNA{4}(random_rna_kmer(len)))
                    for _ in 1:reps
                ],
            )
            @test all(
                Bool[
                    check_longsequence_construction(Kmer, LongDNA{2}(random_dna_kmer(len)))
                    for _ in 1:reps
                ],
            )
            @test all(
                Bool[
                    check_longsequence_construction(Kmer, LongAA(random_rna_kmer(len))) for
                    _ in 1:reps
                ],
            )
            ...
            @test all(
                Bool[
                    check_nucarray_kmer(
                        Kmer{DNAAlphabet{2}, len},
                        random_dna_symbols(len, [0.25, 0.25, 0.25, 0.25, 0.0]),
                    ) for _ in 1:reps
                ],
            )
            @test all(
                Bool[
                    check_nucarray_kmer(Kmer{DNAAlphabet{4}, len}, random_dna_symbols(len))
                    for _ in 1:reps
                ],
            )
            @test all(
                Bool[
                    check_nucarray_kmer(
                        Kmer{RNAAlphabet{2}, len},
                        random_rna_symbols(len, [0.25, 0.25, 0.25, 0.25, 0.0]),
                    ) for _ in 1:reps
                ],
            )
            @test all(
                Bool[
                    check_nucarray_kmer(Kmer{RNAAlphabet{4}, len}, random_rna_symbols(len))
                    for _ in 1:reps
                ],
            )
            @test all(
                Bool[
                    check_nucarray_kmer(AAKmer{len}, random_aa_symbols(len)) for _ in 1:reps
                ],
            )
